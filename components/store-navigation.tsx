      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button asChild size="icon" className="rounded-full shadow-lg relative">
              <Link href="/store" title="Go to Store">
                {cartCount > 0 && (
                  <Badge variant="destructive" className="absolute -top-2 -right-2 z-10 h-5 w-5 flex items-center justify-center p-0">
                    {cartCount}
                  </Badge>
                )}
          ...
          <TooltipContent side="left">
            <p>Web Store{cartCount > 0 ? ` (${cartCount} items in cart)` : ''}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
