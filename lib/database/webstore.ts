...
export async function getCategories(activeOnly = true): Promise<StoreCategory[]> {
...
export async function getCategoryById(id: number): Promise<StoreCategory | null> {
...
export async function createCategory(name: string, description: string | null, orderIndex: number): Promise<number> {
...
export async function updateCategory(id: number, data: Partial<StoreCategory>): Promise<boolean> {
...
export async function deleteCategory(id: number): Promise<boolean> {
  // First update any items in this category to have NULL category_id
  await pool.execute(
    'UPDATE store_items SET category_id = NULL WHERE category_id = ?',
    [id]
  )
...
export async function getItems(activeOnly = true, categoryId?: number): Promise<StoreItem[]> {
...
export async function createItem(data: Partial<StoreItem>): Promise<number> {
  ...
  const query = `
    INSERT INTO store_items (${fields.map(([key]) => key).join(', ')})
...
export async function updateItem(id: number, data: Partial<StoreItem>): Promise<boolean> {
  const fields = Object.entries(data)
    .filter(([key]) => key !== 'id' && key !== 'created_at' && key !== 'updated_at')
...
export async function getPaymentMethods(activeOnly = true): Promise<StorePaymentMethod[]> {
  const query = activeOnly
...
export async function getItemPaymentMethods(itemId: number): Promise<StorePaymentMethod[]> {
  const query = `
...
export async function setItemPaymentMethods(itemId: number, paymentMethodIds: number[]): Promise<boolean> {
  const connection = await pool.getConnection()
...
export async function getDiscounts(activeOnly = true): Promise<StoreDiscount[]> {
  const query = activeOnly
...
export async function getDiscountByCode(code: string): Promise<StoreDiscount | null> {
...
export async function createDiscount(data: Partial<StoreDiscount>): Promise<number> {
  const fields = Object.entries(data)
    .filter(([key]) => key !== 'id' && key !== 'created_at' && key !== 'updated_at')
  
  const query = `
    INSERT INTO store_discounts (${fields.map(([key]) => key).join(', ')})
    VALUES (${fields.map(() => '?').join(', ')})
  `
...
export async function updateDiscount(id: number, data: Partial<StoreDiscount>): Promise<boolean> {
  const fields = Object.entries(data)
    .filter(([key]) => key !== 'id' && key !== 'created_at' && key !== 'updated_at')
...
export async function validateDiscountCode(code: string): Promise<{ valid: boolean, percentage?: number }> {
  const [rows] = await pool.execute<StoreDiscount[]>(
...
export async function useDiscountCode(code: string): Promise<boolean> {
  const [result] = await pool.execute(
    'UPDATE store_discounts SET times_used = times_used + 1 WHERE code = ?',
    [code]
  )
