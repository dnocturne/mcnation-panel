import { stripe, type STRIPE_PAYMENT_CACHE } from "@/lib/stripe";
	...
	try {
		...
		const validItems = items.filter((item) => {
			if (Number.isNaN(item.price) || item.price <= 0) {
				console.warn(
					`Skipping item with invalid price: ${item.id}, ${item.name}, ${item.price}`,
				);
				return false;
			}
			return true;
	...
	try {
		...
		for (const intent of paymentIntents.data) {
			...
			const payment: STRIPE_PAYMENT_CACHE = {
				...
				status: intent.status as StripePaymentStatus, // Type casting to proper status type
			...
			if (intent.payment_method && typeof intent.payment_method === "string") {
				try {
					...
					console.error(
						`Error retrieving payment method for ${intent.id}:`,
						error,
					);
				}
			}
	...
	try {
		...
		if (!successfulPayment) {
			...
			const paymentData: STRIPE_PAYMENT_CACHE = {
				...
				status: latestPayment.status as StripePaymentStatus,
		...
		const paymentData: STRIPE_PAYMENT_CACHE = {
			...
			status: successfulPayment.status as StripePaymentStatus,
...
export async function processStripeEvent(event: Stripe.Event) {
	...
	console.log(`Processing Stripe event: ${type} (${id})`);

	// Handle different event types
	switch (type) {
		case "payment_intent.succeeded":
			// Type assertion to specify the expected object type
			await handlePaymentSuccess(object as Stripe.PaymentIntent);
			break;
		case "checkout.session.completed":
			await handleCheckoutSessionCompleted(object as Stripe.Checkout.Session);
			break;
		case "payment_intent.payment_failed":
		case "payment_intent.canceled":
			await handlePaymentFailure(object as Stripe.PaymentIntent);
			break;
...
/**
 * Handle a payment failure or cancellation
 */
...
/**
 * Get customer ID from a Stripe customer object
 */
