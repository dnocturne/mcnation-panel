...
export async function processCheckoutCompletion(
	...
	try {
		...
		if (!minecraftUsername) {
			console.error(
				"No Minecraft username found in session metadata:",
				sessionId,
			);
			return;
		}
		...
		console.log(
			`Processing checkout for ${minecraftUsername} with ${session.line_items.data.length} items`,
		);
		...
		for (const item of lineItems) {
			try {
				...
				console.log(
					`Granting item: ${product.name} (${item.quantity}x) to ${minecraftUsername}`,
				);
		...
		console.log(
			`Successfully processed checkout ${sessionId} for ${minecraftUsername}`,
		);
	} catch (error) {
