 ...
 */
export async function getStoreItems(activeOnly = true, categoryId?: number): Promise<StoreItem[]> {
 ...
 */
export async function getStoreCategories(activeOnly = true): Promise<StoreCategory[]> {
  try {
    const [rows] = await pool.execute(
