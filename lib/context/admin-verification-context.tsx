...
const AdminVerificationContext = createContext<
	AdminVerificationContextType | undefined
>(undefined);
...
export function AdminVerificationProvider({
	...
	const [lastVerificationTime, setLastVerificationTime] = useState<
		number | null
	>(null);
	const [verificationTimeout, setVerificationTimeout] =
		useState<number>(initialTimeout);
	...
	useEffect(() => {
		...
		if (savedVerification) {
			try {
				...
				const isStillValid =
					verificationTime && Date.now() - verificationTime < savedTimeout;
...
export function useAdminVerification() {
	...
	if (context === undefined) {
		throw new Error(
			"useAdminVerification must be used within an AdminVerificationProvider",
		);
	}
	return context;
}
