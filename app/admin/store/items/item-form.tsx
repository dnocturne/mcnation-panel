...
const formSchema = z.object({
  ...
  sale_price: z.preprocess(
    ...
    z.number().min(0, { message: "Sale price must be 0 or greater" }).nullable()
  ),
                ...
                <Table>
                  ...
                  <TableBody>
                    {paymentMethods.map((method) => {
                      const isSelected = form.getValues().paymentMethodIds?.includes(method.id) || false
                      
                      return (
                        <TableRow key={method.id} className="cursor-pointer hover:bg-muted/50" onClick={() => togglePaymentMethod(method.id)}>
                          <TableCell>
                            <Checkbox
                              checked={isSelected}
