...
export default function PurchasesList() {
  ...
  const exportPurchases = () => {
    try {
      ...
      const headers = "ID,Username,Amount,Discount Code,Discount %,Payment Method,Status,Date\n"
  ...
  const filteredPurchases = filterUsername
    ? purchases.filter(p => p.username.toLowerCase().includes(filterUsername.toLowerCase()))
    : purchases
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <Label htmlFor="filterUsername" className="sr-only">Filter by username</Label>
          <Input
            id="filterUsername"
            ...
            {filteredPurchases.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-6 text-muted-foreground">
                  No purchases found. Purchases will appear here once customers make them.
                </TableCell>
              </TableRow>
            ) : (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">{formatCurrency(selectedPurchase.total_amount)}</p>
                </div>
              </div>
              
              {selectedPurchase.discount_code && (
                <div>
                  <h4 className="text-sm font-medium text-muted-foreground">Discount Applied</h4>
                  <p className="uppercase">{selectedPurchase.discount_code} ({selectedPurchase.discount_percentage}% off)</p>
                </div>
              )}
