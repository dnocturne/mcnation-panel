...
export async function GET(
  ...
  try {
    ...
    const profile = {
      ...
      avatarUrl: fs.existsSync(`public/avatars/${username}.jpg`) ? `/avatars/${username}.jpg` : null
    }
...
// Helper function to determine highest rank (customize based on your rank hierarchy)
function getHighestRank(groups: string[]): string {
  const rankOrder = ['admin', 'operator', 'default']
  for (const rank of rankOrder) {
    if (groups.includes(rank)) {
      return rank
    }
  }
  return 'default'
