...
import { createActionsColumn, createDateCell, createCurrencyCell, createStatusCell, createSortableHeader } from "./columns"
...
const sampleUsers: User[] = Array.from({ length: 50 }).map((_, i) => ({
  ...
  subscription: {
    ...
    renewalDate: new Date(Date.now() + Math.floor(Math.random() * 90) * 86400000).toISOString(),
  }
}))
